...
def generate_report(data, custom_meta=None, last_update_date=None):
    ...
    p.append('    while (modal.firstChild) { modal.removeChild(modal.firstChild); }')
    p.append('    var content = document.createElement("div");')
    p.append('    content.className = "modal-content";')
    p.append('    var header = document.createElement("div");')
    p.append('    header.className = "modal-header";')
    p.append('    var h2 = document.createElement("h2");')
    p.append('    h2.textContent = "Detalhes do Dia " + date;')
    p.append('    header.appendChild(h2);')
    p.append('    var closeBtn = document.createElement("button");')
    p.append('    closeBtn.className = "modal-close";')
    p.append('    closeBtn.innerHTML = "&times;";')
    p.append('    closeBtn.onclick = function() { modal.style.display = "none"; };')
    p.append('    header.appendChild(closeBtn);')
    p.append('    content.appendChild(header);')
    p.append('    var dfInfo = document.createElement("div");')
    p.append('    dfInfo.className = "modal-df-info";')
    p.append('    var dfStrong = document.createElement("strong");')
    p.append('    dfStrong.textContent = "DF do dia: ";')
    p.append('    dfInfo.appendChild(dfStrong);')
    p.append('    dfInfo.appendChild(document.createTextNode(df.toFixed(2) + "%"));')
    p.append('    content.appendChild(dfInfo);')
    p.append('    if (failures.length > 0) {')
    p.append('      var table = document.createElement("table");')
    p.append('      table.className = "modal-failures-table";')
    p.append('      var thead = document.createElement("thead");')
    p.append('      var trh = document.createElement("tr");')
    p.append('      var headers = ["Falha", "Duração", "Equipamento"];')
    p.append('      for (var i = 0; i < headers.length; i++) {')
    p.append('        var th = document.createElement("th");')
    p.append('        th.textContent = headers[i];')
    p.append('        trh.appendChild(th);')
    p.append('      }')
    p.append('      thead.appendChild(trh);')
    p.append('      table.appendChild(thead);')
    p.append('      var tbody = document.createElement("tbody");')
    p.append('      for (var j = 0; j < failures.length; j++) {')
    p.append('        var f = failures[j];')
    p.append('        var tr = document.createElement("tr");')
    p.append('        var td1 = document.createElement("td");')
    p.append('        td1.textContent = f.oque;')
    p.append('        var td2 = document.createElement("td");')
    p.append('        td2.textContent = f.duracao + "h";')
    p.append('        var td3 = document.createElement("td");')
    p.append('        td3.textContent = f.equip;')
    p.append('        tr.appendChild(td1);')
    p.append('        tr.appendChild(td2);')
    p.append('        tr.appendChild(td3);')
    p.append('        tbody.appendChild(tr);')
    p.append('      }')
    p.append('      table.appendChild(tbody);')
    p.append('      content.appendChild(table);')
    p.append('    } else {')
    p.append('      var pEmpty = document.createElement("p");')
    p.append('      pEmpty.style.textAlign = "center";')
    p.append('      pEmpty.style.color = "#666";')
    p.append('      pEmpty.textContent = "Sem falhas registradas neste dia";')
    p.append('      content.appendChild(pEmpty);')
    p.append('    }')
    p.append('    modal.appendChild(content);')
    p.append('    modal.style.display = "block";')
    p.append('    modal.onclick = function(event) { if (event.target == modal) modal.style.display = "none"; };')
    p.append('  } catch(e) { console.error("showDayModal error:", e); }')
    p.append('}')
    p.append('</script>')
